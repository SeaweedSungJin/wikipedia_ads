...
from .embedding import encode_image, get_text_embedding
from .models import get_device, load_image_model, load_text_model, setup_cuda
...
def search_rag_pipeline(cfg: Config) -> Tuple[List[dict], List[dict]]:
    ...
    text_model, tokenizer = load_text_model(device_map="auto")
    ...
    query_emb = get_text_embedding([cfg.text_query], text_model, tokenizer).to(device)
    valid_descriptions = [r["description"] for r in top_k_image_results if r["description"] and not r["description"].isspace()]
    if valid_descriptions:
        desc_embeddings = get_text_embedding(valid_descriptions, text_model, tokenizer).to(device)
    else:
    ...
    section_embeddings = get_text_embedding(section_texts_list, text_model, tokenizer).to(device)
